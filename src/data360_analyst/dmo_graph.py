...
def api_get(token, url, timeout=180):
    req = urllib.request.Request(url, headers={"Authorization": f"Bearer {token}"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return json.loads(r.read())


def fetch_all_cis(token, base, timeout=180):
    items, offset = [], 0
    while True:
        d = api_get(token, f"{base}/ssot/calculated-insights?batchSize=25&offset={offset}", timeout=timeout)
...
def count_streams_from_sidecars(om_dir):
    return sum(1 for _ in (om_dir / "streams").glob("*.yaml"))
...
def main():
    ...
    parser.add_argument("--timeout", type=int, default=180,
                        help="HTTP timeout (seconds) for API calls. Default 180.")
    ...
    cis = fetch_all_cis(token, base, timeout=args.timeout)
    print(f"  {len(cis)} CIs")
    ...
    print("Counting streams from sidecars...")
    stream_count = count_streams_from_sidecars(om_dir)
    ...
    out.append(f"_Generated from intake snapshot. {len(known_dmos)} DMO sidecars • {len(cis)} CIs ({ci_with_no_sql} with no `expression`) • {sum(seg_on.values())} segment→DMO edges • {stream_count} streams (lineage not exposed by API — see note below)._\n")
